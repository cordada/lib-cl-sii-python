...
_BOMS: Sequence[Tuple[bytes, str]] = (
    # warning: order matters, the UTF-32 LE BOM starts with the UTF-16 LE one.
...
def detect_encoding(raw_data: bytes, is_sample: bool = False) -> str:
    ...
    Never raises: the last candidate is an encoding that decodes any byte sequence, so the
    ``RuntimeError`` at the end of the function can only signal a programming error.
