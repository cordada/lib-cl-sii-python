...
def clean_cesiones_periodo_csv_file(
    ...
    * Replace with ``,`` the unescaped ``;`` character used in email fields
    ...
    try:
        ...
        # note: no guard is needed for the case where the detected encoding already is the fallback
        #   one, because reading with it never raises 'UnicodeDecodeError', so this handler cannot
        #   run in that case.
        logger.warning(
...
def _parse_cesiones_periodo_csv_file_with_encoding_fallback(
    ...
    it. In that case the file is parsed again with an encoding that decodes any byte sequence,
    skipping the entries that were already yielded.

    .. warning::
        The fallback encoding is byte-preserving, not meaning-preserving: it maps every byte to a
        character, so nothing is lost, but the non-ASCII characters of the entries yielded by the
        retry come back as mojibake if the file really was in another encoding (e.g.
        ``'CesiÃ³n'`` for a UTF-8 ``'Cesión'``). That is deliberate: decoding with the detected
        encoding and ``errors='replace'`` would instead discard the undecodable bytes
        irrecoverably. The warning that is logged records the row from which the fallback applies,
        so that the affected entries can be identified afterwards.
    ...
    try:
        ...
        if input_file_encoding == LOSSLESS_FALLBACK_ENCODING:
            raise
        logger.warning(
            "Failed to decode file with the detected encoding ('%s') after %s row(s);"
            " retrying with '%s' from row %s onwards: %s",
            input_file_encoding,
            n_yielded_rows,
            LOSSLESS_FALLBACK_ENCODING,
            n_rows_offset + n_yielded_rows + 1,
            input_file_path,
        )

        # note: the rows that were already yielded are skipped through 'n_rows_offset', so they are
        #   not deserialized nor converted a second time (which would, among other things, log
        #   their conversion failures again). 'max_n_rows' is reduced by the same amount so that
        #   the window and the 'MaxRowsExceeded' threshold remain the ones the caller asked for.
        yield from _parse_cesiones_periodo_csv_file(
            ...
            input_file_path,
            LOSSLESS_FALLBACK_ENCODING,
            n_rows_offset + n_yielded_rows,
            max_n_rows - n_yielded_rows if max_n_rows is not None else None,
        )
